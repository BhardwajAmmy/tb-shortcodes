 ...
 */
function tbs_hex_shift( $supplied_hex, $shift_method, $percentage = 50 ) {
 ...
 */
// function tbs_apply_formatting() {
//  // Enable shortcodes in text widgets
//  add_filter( 'widget_text', 'do_shortcode' );
//  // Enable shortcodes in category descriptions
//  add_filter( 'category_description', 'do_shortcode' );
//  // Enable auto-formatting
//  if ( get_option( 'tbs_option_custom-formatting' ) === 'on' ) {
//   // Disable WordPress native content formatters
//   remove_filter( 'the_content', 'wpautop' );
//   remove_filter( 'the_content', 'wptexturize' );
//   // Apply custom formatter function
//   add_filter( 'the_content', 'tbs_custom_formatter', 99 );
//   add_filter( 'widget_text', 'tbs_custom_formatter', 99 );
//   add_filter( 'category_description', 'tbs_custom_formatter', 99 );
//  }
//  // Fix for large posts, http://core.trac.wordpress.org/ticket/8553
//  @ini_set( 'pcre.backtrack_limit', 500000 );
// }

// add_action( 'init', 'tbs_apply_formatting' );
 ...
 */
// function tbs_custom_formatter( $content ) {
//  // Prepare variables
//  $new_content = '';
...
function tbs_apply_formatting() {
	...
	if ( get_option( 'tbs_option_custom-formatting' ) === 'on' ) {
		// Apply custom formatter function
		add_filter( 'the_content', 'tbs_clean_shortcodes' );
	}
}
 ...
 * @return string Formatted content with clean shortcodes content
 */
function tbs_clean_shortcodes( $content ) {
	$p = tbs_cmpt();
	$array = array (
		'<p>[' => '[',
		']</p>' => ']',
 ...
 */
function tbs_do_shortcode( $content, $pre ) {
 ...
 */
function tbs_compatibility_mode_prefix() {
	return get_option( 'tbs_option_prefix' );
}

/**
 * Shortcut for tbs_compatibility_mode_prefix()
 */
function tbs_cmpt() {
	return tbs_compatibility_mode_prefix();
}
 ...
 */
function tbs_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
	global $wp_version;
 ...
 */
function tbs_delete_resized_images( $post_id ) {
...
add_action( 'delete_attachment', 'tbs_delete_resized_images' );

class TBs_Tools {
	function __construct() {
		add_action( 'wp_ajax_tbs_example_preview', array( __CLASS__, 'example' ) );
		add_action( 'tbs/update',                  array( __CLASS__, 'reset_examples' ) );
		add_action( 'tbs/activation',              array( __CLASS__, 'reset_examples' ) );
		add_action( 'techbooth/page/before',        array( __CLASS__, 'reset_examples' ) );
	}
	...
	public static function get_slides( $args ) {
		...
		if ( $args['gallery'] !== null || ( $args['source'] === 'none' && get_option( 'tbs_option_galleries-432' ) ) ) return self::get_slides_432( $args );
	...
	public static function get_slides_432( $args ) {
		...
		$galleries = get_option( 'tbs_option_galleries-432' );
		// No galleries found
	...
	public static function example() {
		...
		// Check for cache
		$output = get_transient( 'tbs/examples/render/' . sanitize_key( $_REQUEST['id'] ) );
		...
		// Cache not found
			...
			// Check for code
			if ( !$code ) die( '<p class="tbs-examples-error">' . __( 'Example code does not found, please check it later', 'tbs' ) . '</p>' );
			// Clean-up the code
			$code = str_replace( array( "\t", '%tbs_' ), array( '  ', tbs_cmpt() ), $code );
			// Split code
			$chunks = explode( '-----', $code );
			// Show snippets
			do_action( 'tbs/examples/preview/before' );
			foreach ( $chunks as $chunk ) {
				...
				echo '<div class="tbs-examples-code"><span class="tbs-examples-get-code button"><i class="fa fa-code"></i>&nbsp;&nbsp;' . __( 'Get the code', 'tbs' ) . '</span><textarea rows="' . $rows . '">' . esc_textarea( $chunk ) . '</textarea></div>';
			}
			do_action( 'tbs/examples/preview/after' );
			$output = ob_get_contents();
			ob_end_clean();
			set_transient( 'tbs/examples/render/' . sanitize_key( $_REQUEST['id'] ), $output );
			echo $output;
		}
		die();
	}
	...
	public static function reset_examples() {
		foreach ( (array) TBs_Data::examples() as $example ) foreach ( (array) $example['items'] as $item ) delete_transient( 'tbs/examples/render/' . $item['id'] );
	}
	...
	public static function icons() {
		$icons = array();
		if ( is_callable( array( 'TBs_Data', 'icons' ) ) ) foreach ( (array) TBs_Data::icons() as $icon ) {
...
/**
 * Just a shortcut for TBs_Tools::decode_shortcode()
 */
function tbs_scattr( $value ) {
	return TBs_Tools::do_attr( $value );
}
