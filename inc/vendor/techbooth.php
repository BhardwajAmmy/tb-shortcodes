<?php
if ( !class_exists( 'Techbooth4' ) ) {

	/**
	 * Techbooth 4
	 *
	 * @author  TechBooth <http://www.techbooth.in>
	 * @version 4
	 */
	class Techbooth4 {
		...
		function __construct( $args = array() ) {
			...
			$args = wp_parse_args( $args, array(
					...
					'css'         => 'assets/css',
					'js'          => 'assets/js',
					'views_class' => 'Techbooth4_Views'
				) );
			// Check required settings
			if ( !$args['file'] ) wp_die( 'Techbooth: please specify plugin __FILE__' );
			if ( !$args['slug'] ) $args['slug'] = sanitize_key( plugin_basename( basename( $args['file'] , '.php' ) ) );
			if ( !$args['prefix'] ) $args['prefix'] = 'techbooth_' . sanitize_key( $args['slug'] ) . '_';
		...
		public function render() {
			...
			do_action( 'techbooth/page/before' );
			echo '<div id="techbooth-settings" class="wrap">';
			...
			echo '<form action="" method="post" id="techbooth-form">';
			echo call_user_func( array( $this->config['views_class'], 'options_page_panes' ), $options, $this->config );
			echo '<input type="hidden" name="techbooth_action" value="save" />';
			do_action( 'techbooth/page/form' );
			echo '</form></div>';
			// Hook after page output
			do_action( 'techbooth/page/after' );
		}
		...
		public function assets() {
			// Register styles
			wp_register_style(  'techbooth', plugins_url( $this->config['css'], $this->config['file'] ) . '/techbooth.css', false, $this->config['version'], 'all' );
			// Register scripts
			wp_register_script( 'form',    plugins_url( $this->config['js'],  $this->config['file'] ) . '/form.js', array( 'jquery' ), $this->config['version'], true );
			wp_register_script( 'techbooth', plugins_url( $this->config['js'],  $this->config['file'] ) . '/techbooth.js', array( 'form' ), $this->config['version'], true );
			// Add some l10n to JS
			wp_localize_script( 'techbooth', 'techbooth', array(
			...
			do_action( 'techbooth/assets/register' );
		}
		...
		public function enqueue() {
			...
			if ( !$this->is_techbooth() ) return;
			// Enqueue styles
			foreach ( array( 'farbtastic', 'techbooth' ) as $style ) wp_enqueue_style( $style );
			// Enqueue scripts
			foreach ( array( 'jquery', 'farbtastic', 'form', 'techbooth' ) as $script ) wp_enqueue_script( $script );
			// Hook to add/remove files
			do_action( 'techbooth/assets/enqueue' );
		}
		...
		public function defaults() {
			...
			if ( get_option( 'techbooth_defaults_' . $this->config['slug'] ) ) return;
			...
			if ( isset( $this->config['options'] ) && is_array( $this->config['options'] ) ) {
				...
				// Defaults is set
				update_option( 'techbooth_defaults_' . $this->config['slug'], true );
			}
		}
		...
		public function submit() {
			// Check request
			if ( empty( $_REQUEST['techbooth_action'] ) || empty( $_REQUEST['page'] ) ) return;
			// Prepare page slug
			$page = sanitize_key( $_GET['page'] );
			// Check page
			if ( !$this->is_techbooth() ) return;
			// Submit hooks
			do_action( 'techbooth/submit', $this );
			do_action( 'techbooth/submit/' . $page, $this );
			// Parse incoming data
			$action  = sanitize_key( $_REQUEST['techbooth_action'] );
			$request = ( isset( $_REQUEST['techbooth'] ) ) ? (array) $_REQUEST['techbooth'] : array();
			// Run actions
			switch ( $action ) {
			case 'save': // Save options
				...
				// Save hooks
				do_action( 'techbooth/save', $this );
				do_action( 'techbooth/save/' . $page, $this );
			...
			case 'reset': // Reset options
				...
				// Reset hooks
				do_action( 'techbooth/reset', $this );
				do_action( 'techbooth/reset/' . $page, $this );
		...
		/**
		 * Conditional check for Techbooth options page
		 *
		 * @return boolean true/false - there is an page created by Techbooth
		 */
		public function is_techbooth() {
...
if ( !class_exists( 'Techbooth4_Views' ) ) {
	 ...
	 */
	class Techbooth4_Views {

		function __construct() {}

		public static function notice( $msg = '', $class = '' ) {
			return '<div class="techbooth-notice ' . $class . '"><p>' . $msg . '</p></div>';
		}

		public static function opentab( $field, $config ) {
			return '<div class="techbooth-pane"><h3 class="hide-if-js techbooth-no-js-tab">' . $field['name'] . '</h3><table class="form-table">';
		}
		...
		public static function closetab( $field, $config ) {
			...
			$return = array();
			$return[] = '</table>';
			if ( $field['actions'] ) $return[] = '<div class="techbooth-actions-bar"><input type="submit" value="' . __( 'Save changes', $config['textdomain'] ) . '" class="techbooth-submit button-primary" /><span class="techbooth-spin"><img src="' . admin_url( 'images/wpspin_light.gif' ) . '" alt="" /> ' . __( 'Saving', $config['textdomain'] ) . '&hellip;</span><span class="techbooth-success-tip"><img src="' . admin_url( 'images/yes.png' ) . '" alt="" /> ' . __( 'Saved', $config['textdomain'] ) . '</span><a href="' . $_SERVER["REQUEST_URI"] . '&amp;techbooth_action=reset" class="techbooth-reset button alignright" title="' . esc_attr( __( 'This action will delete all your settings. Are you sure? This action cannot be undone!', $config['textdomain'] ) ) . '">' . __( 'Restore default settings', $config['textdomain'] ) . '</a></div>';
			$return[] = '</div>';
		...
		public static function text( $field, $config ) {
			...
			return '<tr><th scope="row"><label for="techbooth-field-' . $field['id'] . '">' . $field['name'] . '</label></th><td><input type="text" value="' . get_option( $config['prefix'] . $field['id'] ) . '" name="techbooth[' . $field['id'] . ']" id="techbooth-field-' . $field['id'] . '" class="regular-text" /><p class="description">' . $field['desc'] . '</p></td></tr>';
		}
		...
		public static function textarea( $field, $config ) {
			...
			return '<tr><th scope="row"><label for="techbooth-field-' . $field['id'] . '">' . $field['name'] . '</label></th><td><textarea name="techbooth[' . $field['id'] . ']" id="techbooth-field-' . $field['id'] . '" class="regular-text widefat" cols="' . $field['cols'] . '" rows="' . $field['rows'] . '">' . stripslashes( get_option( $config['prefix'] . $field['id'] ) ) . '</textarea><p class="description">' . $field['desc'] . '</p></td></tr>';
		}
		...
		public static function checkbox( $field, $config ) {
			...
			return '<tr><th scope="row"><label for="techbooth-field-' . $field['id'] . '">' . $field['name'] . '</label></th><td><label><input type="checkbox" name="techbooth[' . $field['id'] . ']" id="techbooth-field-' . $field['id'] . '"' . $checked . ' /> ' . $field['label'] . '</label><span class="description">' . $field['desc'] . '</span></td></tr>';
		}
		...
		public static function checkbox_group( $field, $config ) {
			...
			if ( is_array( $field['options'] ) ) foreach ( $field['options'] as $single ) {
					...
					$group[] = '<label for="techbooth-field-' . $field['id'] . '-' . $single['id'] . '"><input type="checkbox" name="techbooth[' . $field['id'] . '][' . $single['id'] . ']" id="techbooth-field-' . $field['id'] . '-' . $single['id'] . '"' . $checked . ' /> ' . $single['label'] . '</label><br/>';
				}
			return '<tr><th scope="row">' . $field['name'] . '</th><td class="techbooth-checkbox-group">' . implode( '', $group ) . '<span class="description">' . $field['desc'] . '</span></td></tr>';
		}
		...
		public static function media( $field, $config ) {
			...
			return '<tr class="techbooth-media"><th scope="row"><label for="techbooth-field-' . $field['id'] . '">' . $field['name'] . '</label></th><td><input type="text" value="' . get_option( $config['prefix'] . $field['id'] ) . '" name="techbooth[' . $field['id'] . ']" id="techbooth-field-' . $field['id'] . '" class="regular-text techbooth-media-value" /> <a href="javascript:;" class="button techbooth-media-button hide-if-no-js"><img src="' . admin_url( 'images/media-button.png' ) . '" alt="" /> ' . __( 'Media manager', $config['textdomain'] ) . '</a><p class="description">' . $field['desc'] . '</p></td></tr>';
		}
		...
		public static function options_page_tabs( $options, $config ) {
			...
			return ( count( $tabs ) ) ? '<div id="icon-options-general" class="icon32 hide-if-no-js"><br /></div><h2 id="techbooth-tabs" class="nav-tab-wrapper hide-if-no-js">' . implode( '', $tabs ) . '</h2>' : '';
		}
		...
		public static function options_page_notices( $options, $config ) {
			// Setup messsages
			$msgs = apply_filters( 'techbooth/page/notices', array(
		...
		public static function options_page_panes( $options, $config ) {
			...
			foreach ( $options as $option ) {
				...
				// Try to call option from built-in class TechboothX_Views
				...
				// Show error message
				else $panes[] = call_user_func( array( $config['views_class'], 'notice' ), 'Techbooth: ' . sprintf( __( 'option type %s is not callable', $config['textdomain'] ), '<b>' . $option['type'] . '</b>' ), 'error' );
			}
			// Return resulting markup
